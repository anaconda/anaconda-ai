...
2. **Agent env vars**: Per-agent `build_env()` functions. Claude Code uses `ANTHROPIC_BASE_URL` + `ANTHROPIC_API_KEY`. OpenCode uses `OPENCODE_CONFIG_CONTENT` (inline JSON).
