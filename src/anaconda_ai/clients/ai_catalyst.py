...
class AICatalystServers(BaseServers):
    ...
    def _create(
        ...
        if not res.ok:
            try:
                ...
                if "Endpoint Config already exists" in error:
                    msg = "A server with this model and name already exists"
                    raise AnacondaAIException(msg)
            except json.JSONDecodeError:
                pass
        res.raise_for_status()
        data = res.json()["data"]
        ...
        # You cannot issue any actions to this server, like POST .../start,
        # for several seconds after the POST to create the server.
        sleep(5)
    ...
    def _start(self, server_id: str) -> None:
        ...
        if not res.ok:
            msg = json.dumps(res.json().get("error", {}).get("message", ""))
            raise AnacondaAIException(msg)
    ...
    def _delete(self, server_id: str) -> None:
        res = self.client.delete(f"api/ai/inference/servers/server/{server_id}")
        res.raise_for_status()
...
class AICatalystClient(GenericClient):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            server_operations_timeout=server_operations_timeout,
        )
        ai_kwargs: Dict[str, Any] = {}
        if api_version is not None:
            ai_kwargs["api_version"] = api_version

        self.ai_config = self.ai_config.model_copy(update=ai_kwargs, deep=True)

        if self.ai_config.backends.ai_catalyst.api_version is not None:
            self.headers["X-Anaconda-Api-Version"] = (
                self.ai_config.backends.ai_catalyst.api_version
            )
